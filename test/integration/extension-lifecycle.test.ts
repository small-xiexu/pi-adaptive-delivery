...
function candidateRepo(): string {
	...
	writeFileSync(path.join(repo, "docs", "实施计划.md"), "Status: pending\n");
	execFileSync("git", ["add", "tracked.txt", "docs/实施计划.md"], { cwd: repo });
...
function approvalBranch(state: string, sessionId = "fixture-session", cwd = process.cwd()) {
	...
	const solutionContent = [{ type: "text", text: "approved solution" }];
	...
	return [
		...
		},
		{
			...
			data: {
				...
				...(needsPlanApproval
					? {
							planContract: JSON.parse(/```adaptive-delivery-plan\n([\s\S]*?)\n```/.exec(artifact)![1]!),
						}
					: {}),
			},
		},
...
test("rejects plan-derived progress writes from BLOCKED without an implementation approval bundle", async () => {
	...
	await assert.rejects(
		harness.tools.get("delivery_progress_sync").execute(
			"unapproved-progress",
			{ target: "docs/实施计划.md", oldText: "Status: pending", newText: "Status: complete" },
			undefined,
			undefined,
			harness.ctx,
		),
	...
	assert.equal(readFileSync(path.join(repo, "docs", "实施计划.md"), "utf8"), "Status: pending\n");
});
...
test("finalizes only after passed validation and an OK fresh review", async () => {
	...
	assert.equal(persisted.finalEvidence.progressArtifacts[0].path, "docs/实施计划.md");
	await harness.commands.get("delivery-status")?.handler("", harness.ctx);
...
test("syncs only the approved progress target at a writer-free boundary", async () => {
	...
	assert.equal(readFileSync(path.join(repo, "docs", "实施计划.md"), "utf8"), "Status: complete\n");
	assert.deepEqual(harness.ui.statuses.at(-1), ["adaptive-delivery", "验证中 [VALIDATING]"]);
