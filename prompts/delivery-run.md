...
- `implementationWriter=parent`：仅限 `single` 路径。父 Pi 使用源码写入工具实现，完成后调用 `delivery_submit_candidate`。
...
4. 固定验证全部通过后，使用 `delivery_review_candidate` 进行 fresh-context 独立审查。命令失败时先区分候选代码、验证环境和已批准计划：只有代码问题进入返工，计划错误使用 `/delivery-revise`。
...
8. 未授权决策、未知进程、stale candidate 或门禁失败才进入 BLOCKED，并记录可复验的具体原因。
