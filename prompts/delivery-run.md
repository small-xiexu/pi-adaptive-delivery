...
1. `parent` 路径按批准范围直接实现；`worker` 路径把完整批准上下文交给唯一 worker，父 Pi 只监督和裁决。worker 无条件不得修改已批准 solution、plan 或任何 `progressTargets`，这些控制面制品只由父会话在 writer-free 边界同步。
...
7. Standard/High-Risk 在 writer-free 边界按项目规则使用 `delivery_progress_sync` 同步项目进度；Tiny 不注册 progress target。不要把进度文档交给 worker 修改。每次调用前重新读取目标文件的当前 exact 区块，不复用更早同步前保存的 `oldText`；`oldText` 和 `newText` 都必须非空，当前入口不支持删除。若 exact `newText` 已唯一存在，Extension 按幂等成功处理；若 `oldText` 缺失或不唯一且尚未写入，保持当前状态并要求用最新精确片段重试。
