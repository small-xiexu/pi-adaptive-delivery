...
P2.2 已验证内部文档底层与批准核验的组合：复用 Pi 原生编辑，在 I/O 前重新核验批准和父 lease，限制明确 Markdown 路径。取消仍等待在途 I/O 和句柄关闭；部分写入保留，清理失败后拒绝再次写入。正式编辑入口尚未注册，因此当前仍不能通过本 Package 编辑文档。

P2.3 的内部文档批准核验函数在同一次读取中核对本轮确认的正文、来源、Session 文件、当前分支与工作区。新文档请求、生命周期失效或核验失败同步取消旧执行信号，不回退旧范围；活动文档更新本身不撤销原授权。信号不是文件监视器，返回结果不是可缓存的权限令牌。

内部父 writer 已验证获取、原生终态核验及交回：lease 放在当前 worktree 的 Git 元数据目录，不因 Pi 配置目录不同而分裂。仅在文档执行和清理完成、原生工具结果确实落盘后交回；记录、归属或清理不明时保持关闭，reload 不恢复旧 writer。单测使用模拟 TUI，真实 Pi CLI 使用独立测试扩展及假批准来源；这些证据不等于正式入口与真实 TUI 的写入闭环。正式接入与实际工具来源检查仍待完成。
