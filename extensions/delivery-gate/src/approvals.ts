...
const parameters = Type.Object({
	...
	body: Type.String({ minLength: 1, description: "本阶段完整决策正文，用简短分行说明，不复制全文台账。design 先给最新落盘方案的相对路径和本轮修改摘要，再说明目标、范围、关键风险和验收方向；implementation 说明计划修改的文件、步骤、环境、验收和停止条件，并区分计划文件与工具实际可写目录。工具提供原方案查看入口，无须重复抄写；不以路径或摘要 ID 代替决策内容。" }),
	paths: Type.Array(Type.String({ minLength: 1 }), { minItems: 1, description: "design 列明确切的规划 Markdown 路径（方案、唯一台账等），由父维护并禁止子修改；文档编辑默认允许，无须单独授权。implementation 为允许子修改的文件或目录。路径按 cwd 解析，不是 glob。" }),
...
const permissions = {
	design: "确认后编制实施计划；本次不批准源码开发或命令执行。",
...
export function installApprovals(pi: ExtensionAPI) {
	...
	pi.registerCommand("delivery-resume", {
		...
		handler: async (_args, ctx) => {
			...
			try {
				...
				pi.sendUserMessage(`继续审阅当前会话的技术方案。先读取 adaptive-delivery Skill、最新方案文件和唯一台账，核对已有修改意见及现场；再展示当前方案的路径、变化摘要和新的 design 审阅提案。规划文档：${JSON.stringify(proposal.paths)}。以下仅是定位上次讨论的历史提案，不能直接当作最新文件或批准依据。\n\n${proposal.body}\n\n本次只恢复方案审阅，不确认方案或实施；不要自动进入开发。任务内 Markdown 编辑默认允许，直接使用父文档工具，保留用户改动；旧方案和实施批准仍须重新取得。`, { expandPromptTemplates: false });
			} catch (error) {
	...
	pi.registerTool({
		...
		description: "在父 Pi TUI 请求方案审阅或实施确认。Markdown 编辑默认允许，直接使用父文档工具，无须文档授权。方案草稿落盘后用 design 展示，用户可输入意见、确认方案或稍后再看；修改后读回最新文档并发起新提案。两次阶段确认分开，RPC/JSON/print 不接受批准。实施可申请本地 Docker 镜像与只读输入，不授予宿主 Shell。",
		parameters,
		execute: async (toolCallId, request, signal, _onUpdate, ctx) => {
			...
			try {
				...
				if (!paths.length) throw new Error("方案须列明规划文档；实施须列明开发路径");
				...
				if (request.stage === "design" && typeof choice === "object" && choice.feedback.trim()) {
					...
					current();
					return { content: [{ type: "text", text: `用户对本次方案的修改意见：\n${choice.feedback}\n\n尚未确认方案。先读最新方案并修订同一份文件，Markdown 编辑无需额外授权；需澄清时一次只问一个关键问题。说明修改与未采纳原因，再发起新的 design 审阅提案，继续等待意见或明确批准。不要进入实施计划或开发。` }],
				...
				if (choice !== accept) return { content: [{ type: "text", text: request.stage === "design"
					? "方案审阅已暂停，文档与已发送意见保留。用户可输入继续看方案、直接提出意见，或用 /delivery-resume 恢复；现在停止推进，不自动重问。"
				...
				return { content: [{ type: "text", text: `${titles[request.stage]}已记录。${request.stage === "implementation" ? `用户未要求暂停且没有未决问题时，继续在本轮文档边界和 writer 交接下委派开发，无须额外的“继续”；${container ? "仅按已确认镜像和挂载执行容器命令；固定验收使用本次命令清单。" : "本次不授权容器命令，固定验收不可用。"}独立审查仍需当前候选的可信验收，不开放宿主 Shell。` : "用户未要求暂停且没有未决问题时，继续编制实施计划；Markdown 编辑默认允许，实施仍须独立确认。"}` }],
