...
export function assertReadOnlyEnvironment(expected: ReadOnlyEnvironment, actual?: ReadOnlyEnvironment): void {
	...
	if (expected.rules !== actual?.rules) throw new Error("项目或全局规则未对齐；请核对配置与已加载内容。未发送任务" + beforeSend);
	if (expected.skills !== actual?.skills) throw new Error("Skills 目录、来源或描述未对齐；请按配置提供子任务所需 Skill。未发送任务" + beforeSend);
}
